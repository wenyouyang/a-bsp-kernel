...
static int tdf8532_stop_play(struct tdf8532_priv *tdf8532)
{
	...
	/*
	 * FIXME: we found stop failed issue on SOF, to work
	 * around it, we still continue the stop(disconnect) even it
	 * doesn't go to STATE_STBY, to work around the next
	 * start_play failed issue.
	 * May need root cause why the timeout happen and fix
	 * it, then still 'goto out' here after that.
	 */
