...
static int __mxt_read_927INTB_reg(struct i2c_client *client,
...
static irqreturn_t mxt_interrupt(int irq, void *dev_id)
{
	...
	/*hs7 special code*/
	if (1 /* data->pdata->gpio_reset == 460 || data->pdata->gpio_reset == 446 */)
	{
		ret =__mxt_read_927INTB_reg(data->client,0xc7,1,&temp);
		if (ret) 
		{
			printk("Failed to clear INTRB 927 isr\n");
		}
		else{
		//dev_warn(dev, "clear INTRB 927 isr=0x%.2X\n",temp); 
		//ret =__mxt_read_927INTB_reg(data->client,0xc6,1,&temp);
		//dev_warn(dev, "clear INTRB 927 isr=0x%.2X\n",temp);
		}
	}
...
static int mxt_acquire_irq(struct mxt_data *data)
{
	...
	if (!data->irq) {
		printk("ADEEL %s: acquiring irq %d trigger 0x%x  gpio_value %d\n", __FUNCTION__, data->client->irq, data->pdata->irqflags, gpio_get_value(455));
...
static int mxt_initialize(struct mxt_data *data)
{
	...
	/*{
		u16 r = 0xC60C;
		mxt_write_reg(data->client, r, 0x21);
	}*/
 ...
 *  HS7 special code
 *************************************************************************/
#define IVI_TOUCH_RESET             464 /* GPIO_30 */
#define IVI_TOUCH_INTR              455 /* GPIO_21 */
#define RSE_LEFT_TOUCH_RESET        460 /* GPIO_26 */
#define RSE_LEFT_TOUCH_INTR         456 /* GPIO_22 */
#define RSE_RIGHT_TOUCH_RESET       446 /* GPIO_12 */
#define RSE_RIGHT_TOUCH_INTR        452 /* GPIO_18 */
...
static struct mxt_platform_data ivi_touch_plat = {
	//.irqflags = IRQF_TRIGGER_FALLING,
	.irqflags = IRQF_TRIGGER_LOW,
	.gpio_reset = IVI_TOUCH_RESET,
	.gpio_irq = IVI_TOUCH_INTR,
};

static struct i2c_board_info ivi_touch_info = {
	.type = "atmel_mxt_ts",
	.addr = 0x94 >> 1,
	.platform_data = &ivi_touch_plat,
};

static struct mxt_platform_data rse_left_touch_plat = {
	.irqflags = IRQF_TRIGGER_LOW,
	.gpio_reset = RSE_LEFT_TOUCH_RESET,
	.gpio_irq = RSE_LEFT_TOUCH_INTR,
};

static struct i2c_board_info rse_left_touch_info = {
	.type = "atmel_mxt_ts",
	.addr = 0x94 >> 1,
	.platform_data = &rse_left_touch_plat,
};

static struct mxt_platform_data rse_right_touch_plat = {
	.irqflags = IRQF_TRIGGER_LOW,
	.gpio_reset = RSE_RIGHT_TOUCH_RESET,
	.gpio_irq = RSE_RIGHT_TOUCH_INTR,
};

static struct i2c_board_info rse_right_touch_info = {
	.type = "atmel_mxt_ts",
	.addr = 0x94 >> 1,
	.platform_data = &rse_right_touch_plat,
};

static struct mxt799t_info mxt799t_devices[] = {
    { "IVI TOUCH",       2, &ivi_touch_info,       NULL },
};


static void unregist_mxt799t_device( struct mxt799t_info *info )
{
	i2c_unregister_device( info->i2c_client );
}

static int regist_mxt799t_device( struct mxt799t_info *info )
{
	struct i2c_adapter	*adapter;
    struct mxt_platform_data  *plat;
 
     
    adapter = i2c_get_adapter( info->bus_id );
    if( !adapter ){
        printk( "MXT: failed to get I2C adapator %d\n", info->bus_id );
        goto adapter_failed;
    }

    plat = info->board_info->platform_data;

    info->board_info->irq = gpio_to_irq( plat->gpio_irq );

    info->i2c_client = i2c_new_device( adapter, info->board_info );
    if( !info->i2c_client ){
        printk( "MXT: failed to regist ivi_touch device\n" );
        goto ivi_device_failed;
    }
...
static void sleep_mxt799t_device( struct mxt799t_info *info )
{
    struct mxt_platform_data  *plat = info->board_info->platform_data;

    gpio_direction_output( plat->gpio_reset, 0 );
    gpio_free( plat->gpio_irq   );
    gpio_free( plat->gpio_reset );
}

static int wake_mxt799t_device( struct mxt799t_info *info )
{
    int res;
    struct mxt_platform_data *p;
    char name[ 256 ];

    p = info->board_info->platform_data;

    /* GPIO RESET */
    if( !gpio_is_valid( p->gpio_reset ) ){
		printk( "MXT: GPIO RESET %ld is invalid\n", p->gpio_reset );
		goto gpio_reset_invalid;
    }

    snprintf( name, 256, "%s Reset", info->name );

    printk( "MXT: setting GPIO RESET %ld\n", p->gpio_reset );
    res = gpio_request( p->gpio_reset, name );
    if( res ){
        printk( "can't request GPIO RESET %ld\n", p->gpio_reset );
        goto gpio_reset_request_failed;
    }

    gpio_direction_output( p->gpio_reset, 1 );

    /* GPIO IRQ */  
    if( !gpio_is_valid( p->gpio_irq ) ){
		printk( "MXT: GPIO IRQ %ld is invalid\n", p->gpio_irq );
		goto gpio_irq_invalid;
    }

    snprintf( name, 256, "%s Interrupt", info->name );

    printk( "MXT: setting GPIO INT %ld\n", p->gpio_irq );
    res = gpio_request( p->gpio_irq, name );
    if( res ){
        printk( "can't request GPIO IRQ %ld\n", p->gpio_irq );
        goto gpio_irq_request_failed;
    }

    gpio_direction_input( p->gpio_irq );
    printk( "MXT: GPIO INT %ld value %d\n", p->gpio_irq, gpio_get_value(p->gpio_irq));

    return 0;

gpio_irq_request_failed:
gpio_irq_invalid:
    gpio_free( p->gpio_reset );
gpio_reset_request_failed:
gpio_reset_invalid:
    return -1;
}
#endif

struct i2c_client *i2c_bus = NULL;
...
static int __init mxt799T_init(void)
{
	int i, ret;
	#if 1
    struct mxt799t_info *info;
	
    /* wakeup device */
    for( i=0; i< ARRAY_SIZE( mxt799t_devices ); i++ ){

        info = &mxt799t_devices[ i ];
        ret = wake_mxt799t_device( info );
        if( ret ){
            break;
        }
    }

    if( i < ARRAY_SIZE( mxt799t_devices ) ){
        for( ; i; i-- ){
            info = &mxt799t_devices[ i - 1 ];
            sleep_mxt799t_device( info );
        }
        goto wake_failed;
    }
    ...
    for( i=0; i< ARRAY_SIZE( mxt799t_devices ); i++ ){
        ...
        info = &mxt799t_devices[ i ];
        ret = regist_mxt799t_device( info );
        if( ret ){
            break;
        }
    }
	...
	#endif
	/* regist driver */
	//ret = i2c_register_driver( THIS_MODULE, &mxt_driver ); 

	i2c_for_each_dev("i2c-2", find_i2c_bus);

	if(i2c_bus == NULL)
		printk("MXT: i2c_bus is NULL\n");
	else
		ret = mxt_probe(i2c_bus, NULL);

	if( ret ){
		printk( "MXT: failed to regist mxt_driver\n" );
		goto driver_failed;
	}
	
	printk( "MXT: module init finish\n" );
    return 0;
#if 1
driver_failed:

    printk( "MXT: failed driver_failed\n" );
    for( i=0; i< ARRAY_SIZE( mxt799t_devices ); i++ ){
        info = &mxt799t_devices[ i ];
        unregist_mxt799t_device( info );
    }
...
device_failed:
    ...
    printk( "MXT: failed device_failed\n" );
    for( i=0; i< ARRAY_SIZE( mxt799t_devices ); i++ ){
        info = &mxt799t_devices[ i ];
        sleep_mxt799t_device( info );
    }
...
wake_failed:
        ...
        printk( "MXT: failed wakeup failed\n" );
	return -1;
	#endif
}

static void __exit mxt799T_exit(void)
{
    int i;
    struct mxt799t_info *info;
...
module_init(mxt799T_init);
module_exit(mxt799T_exit);
